...
export function filterModernPlayers(data: GraphData): GraphData {
  ...
  const modernEdges = data.edges.filter(edge => 
    modernNodeIds.has(edge.source) && modernNodeIds.has(edge.target)
  );
...
export interface GraphInstance {
  ...
  forEachEdge: (callback: (edgeId: string, attributes: { source: string; target: string }, source: string, target: string, sourceBag: unknown, targetBag: unknown) => void) => void;
}
