...
import type { NodeData, GraphData, TeamTenure } from "@/lib/graph-types";
...
function getTopTeammates(playerId: string, graphData: GraphData | null, limit = 5): TeammateInfo[] {
  ...
  const teammateMap = new Map<
    string,
    { teammateId: string; totalDays: number; bestTeam: TeamTenure }
  >();
  ...
  for (const edge of connectedEdges) {
    ...
    if (!existing || existing.totalDays < edge.total_days) {
      const bestTeam = edge.teams.reduce<TeamTenure>(
        (best, t) => (t.overlap_days > best.overlap_days ? t : best),
        firstTeam,
      );
  ...
  return teammates.sort((a, b) => b.totalDays - a.totalDays).slice(0, limit);
}
...
export default function PlayerTooltip({
  ...
  return (
    ...
    >
      ...
      <div style={statStyle}>Degree Centrality: {player.degree}</div>
        ...
        <>
          ...
          <div style={teammatesListStyle}>
            {teammates.map((teammate, index) => (
              <div key={teammate.id} data-testid={`teammate-${index}`} style={teammateItemStyle}>
      ...
      {highlightedNode === player.id && <div style={highlightHintStyle}>Click again to reset</div>}
    </div>
  );
}
