...
const createMockSigma = () => {
  ...
  return {
    getGraph: () => ({
      ...
      hasEdge: (source: string, target: string) => edges.some(e => e.source === source && e.target === target),
      forEachEdge: (callback: (edgeId: string, attrs: { source: string; target: string }, source: string, target: string) => void) => {
        edges.forEach(e => { callback(e.id, e, e.source, e.target); });
      },
    }),
...
const createMockGraphData = (): GraphData => ({
  nodes: [
    { id: 'n1', label: 'Michael Jordan', x: 0, y: 0, size: 20, color: '#ff0000', position: 'SG', era: 'modern', is_active: false, hof: true, draft_year: 1984, degree: 5, betweenness: 0.5, community: 1 },
    { id: 'n2', label: 'Scottie Pippen', x: 100, y: 100, size: 18, color: '#00ff00', position: 'SF', era: 'modern', is_active: false, hof: true, draft_year: 1987, degree: 4, betweenness: 0.3, community: 1 },
    { id: 'n3', label: 'Bill Russell', x: -100, y: -100, size: 16, color: '#0000ff', position: 'C', era: 'classic', is_active: false, hof: true, draft_year: 1956, degree: 3, betweenness: 0.2, community: 2 },
    { id: 'n4', label: 'Wilt Chamberlain', x: 200, y: 200, size: 22, color: '#ffff00', position: 'C', era: 'classic', is_active: false, hof: true, draft_year: 1959, degree: 2, betweenness: 0.1, community: 2 },
    { id: 'n5', label: 'LeBron James', x: 300, y: 300, size: 21, color: '#ff00ff', position: 'SF', era: 'modern', is_active: true, hof: false, draft_year: 2003, degree: 6, betweenness: 0.6, community: 1 },
  ],
  edges: [
...
describe('filterModernPlayers', () => {
  ...
  test('handles null draft_year', () => {
    const data: GraphData = {
      nodes: [
        { id: 'n1', label: 'Unknown', x: 0, y: 0, size: 10, color: '#000', position: 'PG', era: 'unknown', is_active: false, hof: false, draft_year: null, degree: 1, betweenness: 0, community: 1 },
        { id: 'n2', label: 'Modern', x: 1, y: 1, size: 10, color: '#000', position: 'PG', era: 'modern', is_active: false, hof: false, draft_year: 2000, degree: 1, betweenness: 0, community: 1 },
      ],
      edges: [{ id: 'e1', source: 'n1', target: 'n2', weight: 1, teams: [], total_days: 100, size: 1 }],
    };
...
describe('Viewport Culling', () => {
  test('creates viewport culler with sigma instance', () => {
    const sigma = createMockSigma() as any;
  ...
  test('getVisibleNodes returns only nodes within viewport', () => {
    ...
    sigma.getViewport = () => mockViewport;
