...
const TEAM_ABBREVIATIONS = [
  ...
  'LAL', 'MIA', 'MIL', 'MIN', 'BKN', 'NYK', 'ORL', 'IND', 'PHI', 'TOR',
  'UTA', 'POR', 'SAC', 'SAS', 'OKC', 'PHX', 'MEM', 'CHA', 'DET', 'WAS'
];
...
const createMockEdge = (id: string, source: string, target: string, teams: TeamTenure[] = []): EdgeData => ({
  id,
  source,
  target,
  weight: 1,
...
describe('Filters Component', () => {
  ...
  describe('Filter Integration with Graph', () => {
    it('passes filtered data to onFilterChange callback', () => {
      ...
      const filteredData = createMockGraphData([createMockNode('1', { era: '1990s', position: 'SG' })]);
      mockFilterByEra.mockReturnValue(filteredData);
      ...
      expect(onFilterChange).toHaveBeenCalledWith(expect.objectContaining({
        nodes: expect.arrayContaining([
          expect.objectContaining({ era: '1990s' })
        ])
      }));
    });
